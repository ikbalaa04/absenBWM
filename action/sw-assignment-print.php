...
if(empty($_COOKIE['COOKIES_MEMBER']) || empty($_COOKIE['COOKIES_COOKIES']) || empty($row_user['id'])){
  header('location:../');
  exit();
}
...
if (empty($error)) {
  ...
  $query="SELECT assignments.*,employees.employees_name,employees.employees_code,position.position_name FROM assignments INNER JOIN employees ON employees.id=assignments.employees_id INNER JOIN position ON position.position_id=employees.position_id WHERE assignments.assignment_id='$id' AND assignments.employees_id='$employees_id'";
...
<head>
  ...
  <style>
    ...
    h2,h3{margin:0;line-height:1.35}.header{border-bottom:2px solid #000;padding-bottom:12px;margin-bottom:30px}
...
<section class="container">
  ...
  <div class="content">
    ...
    <div class="signature">
      ...
      <table>
        <tr>
          <td class="text-center">Menugaskan<p class="name">'.$site_manager.'</p></td>
